...
{  
  ...
  while(IsMotorEnable) 
	...
	speed=.5;
        ...
        std::cout << "total distance of left + right " << totalDist << std::endl;
      //if total distance of left + right greater than 2.4 turn to the side have bigger value 
         if(totalDist > 2.5){
      ...
      //if robot  reach the  top left  stop itself
      if(pp.GetXPos() < 4 && pp.GetYPos() > 7)
      {
	IsMotorEnable = false;
      }         
